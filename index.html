...
<head>
  ...
  <style>
    ...
    nav {
      background: #333; color: #fff; width: 200px; height: 100vh;
      padding: 20px; box-sizing: border-box;
    }
    nav h3 { margin-top: 0; }
    nav a {
      color: #fff; display: block; margin: 10px 0;
      text-decoration: none; cursor: pointer;
    }
    nav a.active { font-weight: bold; text-decoration: underline; }
    .content { flex: 1; padding: 20px; }
    .eye-global {
      cursor: pointer; font-size: 20px; margin-bottom: 10px; display: inline-block;
    }
    .cards { display: flex; gap: 20px; margin-bottom: 20px; flex-wrap: wrap; }
    .card {
      background: #fff; padding: 20px; flex: 1 1 200px;
      border-radius: 8px; box-shadow: 0 0 8px rgba(0,0,0,0.1);
      text-align: center;
    }
    table {
      width: 100%; border-collapse: collapse; background: #fff; overflow-x: auto;
    }
    th, td {
